...
// Output : AI_Response
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakFindResponse( AI_Response &outResponse, AIConcept_t ai_concept, const char *modifiers /*= NULL*/ )
{
	...
	set.AppendCriteria( "concept", ai_concept, CONCEPT_WEIGHT );
...
// Input  : *response - 
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakDispatchResponse( AIConcept_t ai_concept, AI_Response& response, IRecipientFilter *filter /* = NULL */ )
{
	bool spoke = false;
	...
	if ( IsSpeaking() && ai_concept[0] != 0 )
	{
		DevMsg( "SpeakDispatchResponse:  Entity ( %i/%s ) already speaking, forcing '%s'\n", GetOuter()->entindex(), STRING( GetOuter()->GetEntityName() ), ai_concept );
	...
	if ( spoke )
	{
		...
		if ( DebuggingSpeech() && g_pDeveloper->GetInt() > 0 && response.GetType() != RESPONSE_PRINT )
		{
			Vector vPrintPos;
			GetOuter()->CollisionProp()->NormalizedToWorldSpace( Vector(0.5,0.5,1.0f), &vPrintPos );
			NDebugOverlay::Text( vPrintPos, CFmtStr( "%s: %s", ai_concept, szResponse ), true, 1.5 );
		}
		...
		SetSpokeConcept( ai_concept, &response );
	}
...
//-----------------------------------------------------------------------------
bool CAI_Expresser::Speak( AIConcept_t ai_concept, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /* = NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
    AI_Response response;
	bool result = SpeakFindResponse( response, ai_concept, modifiers );
	if ( !result )
		return false;

	SpeechMsg( GetOuter(), "%s (%p) spoke %s (%f)\n", STRING(GetOuter()->GetEntityName()), GetOuter(), ai_concept, gpGlobals->curtime );

	bool spoke = SpeakDispatchResponse( ai_concept, response, filter );
...
//-------------------------------------
bool CAI_Expresser::CanSpeakConcept( AIConcept_t ai_concept )
{
	// Not in history?
	int iter = m_ConceptHistories.Find( ai_concept );
...
bool CAI_Expresser::SpokeConcept( AIConcept_t ai_concept )
{
	return GetTimeSpokeConcept( ai_concept ) != -1.f;
}

//-------------------------------------

float CAI_Expresser::GetTimeSpokeConcept( AIConcept_t ai_concept )
{
	int iter = m_ConceptHistories.Find( ai_concept );
	if ( iter == m_ConceptHistories.InvalidIndex() ) 
		return -1;
...
void CAI_Expresser::SetSpokeConcept( AIConcept_t ai_concept, AI_Response *response, bool bCallback )
{
	int idx = m_ConceptHistories.Find( ai_concept );
	if ( idx == m_ConceptHistories.InvalidIndex() )
	{
		ConceptHistory_t h;
		h.timeSpoken = gpGlobals->curtime;
		idx = m_ConceptHistories.Insert( ai_concept, h );
	}
	...
	if ( bCallback )
		GetSink()->OnSpokeConcept( ai_concept, response );
}
