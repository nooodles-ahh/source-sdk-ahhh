...
bool CAI_LeadBehavior::Speak( AIConcept_t ai_concept )
{
	CAI_Expresser *pExpresser = GetOuter()->GetExpresser();
	if ( !pExpresser )
		return false;
	...
	bool bNag = ( FStrEq(ai_concept,TLK_LEAD_COMINGBACK) || FStrEq(ai_concept, TLK_LEAD_CATCHUP) || FStrEq(ai_concept, TLK_LEAD_RETRIEVE) );
	...
	if ( pExpresser->Speak( ai_concept, GetConceptModifiers( ai_concept ) ) )
	{
